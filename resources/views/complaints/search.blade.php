...
@section('content')
    <div class="grid grid-cols-4">
        <div class="shadow-xl border p-4 col-span-1">
            <h1 class="font-semibold text-xl">Recherche par Date</h1>
            <form action="{{ route('complaint.search') }}" method="GET">
                @csrf
                <div class="md:flex md:justify-around my-5">
                    <div>
                        <label for="">Date Debut : </label>
                        <input type="date" name="dateDebut" value="{{(new DateTime(now()))->format('d-m-Y')}}" class="rounded-sm block">
                    </div>
                    <div>
                        <label for="">Date Fin : </label>
                        <input type="date" name="dateFin" class="rounded-sm block">
                    </div>
                </div>
                <div class="ml-1">
